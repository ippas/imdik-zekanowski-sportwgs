...
#############################################################
# This code is responsible for analysis weightlifters versus sportsmen
...
df_top_weights_sportsmen %>% 
  ...
  mutate(boxplot = map2(
    .x = data,
    .y = stat_signif_df,
    ~ boxplot_prs(.x, size = 30, stat_signif_df = .y, levels = c("weights", "sportsman", "control", "polish_control"))
  )) %>%
