...
prs_results_with_sport %>%
  ...
  statistics_sportsmen_prs(column_group = "group", group1 = experiment_group, group2 = control_group) -> prs_swim_weights
...
prs_statistic_preprocessing(prs_swim_weights) %>% 
  ...
  mutate(group = ifelse(sport == experiment_group | sport == control_group, sport, group)) -> df_top_swim_weights
...
wrap_plots(swim_weighs_to_plot$plot) +
  plot_annotation(title = "277 Origin Categories",
                  theme = theme(plot.title = element_text(size = 20))) +
  plot_layout(
    ncol = 1,
    guides = "collect",
    heights =  unlist(swim_weighs_to_plot$heights)
  ) & theme(legend.position = 'bottom') -> swim_weighs_plot
...
number_group(df_top_swim_weights) -> df_number_group
...
#                   params = list(plot = swim_weighs_plot,
#                                 df_info = df_info,
...
df_top_swim_weights %>% 
  ...
  mutate(boxplot = map2(
    .x = data,
    .y = stat_signif_df,
    ~ boxplot_prs(.x, size = 30, stat_signif_df = .y, levels = c("swim", "weights", "control", "polish_control", "sportsman"))
  )) %>%
...
png("analysis/reports-prs/swim-weights.png", width = 1920, height = 1080)
